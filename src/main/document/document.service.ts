...
export class DocumentService {
    ...
    async createDocService(createDocFileDto: CreateDocFileDto) {
        ...
        if(user && email === user.email) {
            throw new BadRequestException(`Email ${email} đã được sử dụng cho tài khoản khác, Vui lòng nhập email khác.`)
        }
        ...
        if (result) {
            ...
            console.log("=======>", valueTextEail);
            // Send mail
            this.mailerService.sendMail({
                to: email, // list of receivers
                from: 'nghiepnq@digitechglobalco.com', // sender address
                ...
                html: isUser ? `<h1>Tao hồ sơ thành công </h2>
                <br/>
